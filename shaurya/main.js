...
if (yearDiff > 0) {
  counter.textContent = yearDiff === 1 ? "🎉 Happy 1st Birthday! 🎉" :
    yearDiff === 2 ? "🎈 Happy 2nd Birthday! 🎈" :
...
slideshowImages.slice(1).forEach(src => { const i = new Image(); i.src = src; });

(function runSlideshow(){
  const img = document.querySelector('.photo');
  if(!img || slideshowImages.length < 2) return;
  let index = 0;
  const SHOW_MS = 2500;
  const FADE_MS = 300;
  setInterval(() => {
